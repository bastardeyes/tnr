...
class Tenorion:
    def __init__(self):
        pygame.init()
        pygame.midi.init()
        port = pygame.midi.get_default_output_id() # 1 #yamaha ux-16(usb)
    ...
    def remote_on(self,is_intialize=True,is_play=True):
        ...
        midi_list= [0xF0, 0x43, 0X73, 0x01, 0x33, 0x01, 0x00, 0x00, 0x01, 0x00, 0x00, 0x00, 0x00, 0xF7]
        if is_intialize!=True:
            midi_list=[0xF0, 0x43, 0X73, 0x01, 0x33, 0x01, 0x00, 0x00, 0x02, 0x00, 0x00, 0x00, 0x00, 0xF7]

        if is_play==True:
            midi_out.write_sys_ex(pygame.midi.time(), midi_list)
        return midi_list


    def remote_off(self,is_play=True):
        midi_out=self.midi_out
        midi_list= [0xF0, 0x43, 0X73, 0x01, 0x33, 0x01, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0xF7]
        if is_play==True:
            midi_out.write_sys_ex(pygame.midi.time(),midi_list)
        return midi_list
    ...
    def play(self,is_play=True):
        midi_out=self.midi_out
        midi_list= [0xF0, 0x43, 0X73, 0x01, 0x33, 0x01, 0x00, 0x08, 0x01, 0x00, 0x00, 0x00, 0x00, 0xF7]
        if is_play==True:
            midi_out.write_sys_ex(pygame.midi.time(),midi_list)
            #print("aaaaa")
        return midi_list
    ...
    def pause(self,is_play=True):
        midi_out=self.midi_out
        midi_list=[0xF0, 0x43, 0X73, 0x01, 0x33, 0x01, 0x00, 0x08, 0x00, 0x00, 0x00, 0x00, 0x00, 0xF7]
        if is_play==True:
            midi_out.write_sys_ex(pygame.midi.time(), midi_list)
        return midi_list
    ...
    def clear_all_layer(self,layer=0x11,is_play=True):
        ...
        block=0x00
        #layer=0x11
        midi_list=[0xF0, 0x43, 0X73, 0x01, 0x33, 0x01, 0x00, 0x0A, block, layer, 0x00, 0x00, 0x00, 0xF7]
        if is_play==True:
            midi_out.write_sys_ex(pygame.midi.time(), midi_list)
        return midi_list
    ...
    def clear_all_block(self,block=0x0A,is_play=True):
        ...
        block=0x0A
        layer=0x0A
        midi_list=[0xF0, 0x43, 0X73, 0x01, 0x33, 0x01, 0x00, 0x0A, block, layer, 0x01, 0x01, 0x00, 0xF7]
        if is_play==True:
            midi_out.write_sys_ex(pygame.midi.time(),midi_list)
        return midi_list
    ...
    def change_loop_indicator_position(self,play_point=0x11,is_play=True):
        #midi_out=self.midi_out
        midi_list=[0xF0, 0x43, 0X73, 0x01, 0x33, 0x01, 0x00, 0x09, play_point, 0x00, 0x00, 0x00, 0x00, 0xF7]
        #if is_play==True:
        #    midi_out.write_sys_ex(pygame.midi.time(),midi_list )
        return midi_list
    ...
    def led_on_off(self,on_off=False,x=0x00,y=0x00,layer=0x00,is_play=True):
        ...
        if on_off==True:
            led_mode=0x02 #on
        midi_out=self.midi_out
        midi_list=[0xF0, 0x43, 0X73, 0x01, 0x33, 0x01, 0x00, led_mode, x, y, layer, 0x00, 0x00, 0xF7]
        if is_play==True:
            midi_out.write_sys_ex(pygame.midi.time(),midi_list )
        return midi_list
    ...
    def led_hold_on(self,x=0x00,y=0x00,layer=0x00,is_play=True):
        midi_out=self.midi_out
        mode=0x00
        midi_list=[0xF0, 0x43, 0X73, 0x01, 0x33, 0x01, 0x00, 0x06, 0x00, 0x00, 0x02, 0x00, 0x00, 0xF7]
        if is_play==True:
            midi_out.write_sys_ex(pygame.midi.time(),midi_list )
        return midi_list
    ...
    def led_hold_on_off(self,on_off=False,x=0x00,y=0x00,layer=0x00,is_play=True):
        ...
        if on_off==True:
            led_mode=0x00 #on
        # print((x, y, layer, led_mode))
        midi_list= [0xF0, 0x43, 0X73, 0x01, 0x33, 0x01, 0x00, 0x06, x, y, layer, led_mode, 0x00, 0xF7]
        if is_play==True:
            midi_out.write_sys_ex(pygame.midi.time(), midi_list)
        return midi_list
    ...
    def current_block_change(self,block=0x00,is_play=True):
         # block = 0 to 15
        midi_out=self.midi_out
        midi_list= [0xF0, 0x43, 0X73, 0x01, 0x33, 0x01, 0x00, 0x0F, block, 0x00, 0x00, 0x00, 0x00, 0xF7]
        if is_play==True:
            midi_out.write_sys_ex(pygame.midi.time(),midi_list)
        return midi_list
    ...
    def current_layer_change(self,layer=0x00,is_play=True):
        midi_out=self.midi_out
        midi_list=[0xF0, 0x43, 0X73, 0x01, 0x33, 0x01, 0x00, 0x10, layer, 0x00, 0x00, 0x00, 0x00, 0xF7]
        if is_play==True:
            midi_out.write_sys_ex(pygame.midi.time(),midi_list )
        return midi_list
        """
    ...
    def layer_parameter_change(self,param_id,d1,d2,layer=0x00,is_play=True):    
        print((param_id, d1, d2, layer)  )
        midi_out=self.midi_out
        midi_list=[0xF0, 0x43, 0X73, 0x01, 0x33, 0x01, 0x00, 0x0D, param_id, d1, d2, layer, 0x00, 0xF7]
        if is_play==True:
            midi_out.write_sys_ex(pygame.midi.time(), midi_list)
        return midi_list
        """
    ...
    def common_param_change(self,param_id,d1,d2,is_play=True):
        midi_out=self.midi_out
        midi_list= [0xF0, 0x43, 0X73, 0x01, 0x33, 0x01, 0x00, 0x0C, param_id, d1, d2, 0x00, 0x00, 0xF7]
        if is_play==True:
            midi_out.write_sys_ex(pygame.midi.time(),midi_list)
        return midi_list
        """
    ...
    def execute_midi_lists(self,midi_lists):
        midi_out=self.midi_out
        for x in midi_lists:
            midi_out.write_sys_ex(pygame.midi.time(),x)
